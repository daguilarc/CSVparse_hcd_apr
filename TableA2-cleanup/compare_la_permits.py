...
# Step 2: Filter to Los Angeles only
df_la = df[df[JURIS_COL].str.strip().str.upper() == 'LOS ANGELES'].copy()
print(f"Los Angeles rows: {len(df_la):,}")

# Step 3: Date-year validation
iss_mismatch = df_la.apply(lambda r: check_date_year_mismatch(r, YEAR_COL, ISS_DATE_COL, PERMITS_COL), axis=1)
ent_mismatch = df_la.apply(lambda r: check_date_year_mismatch(r, YEAR_COL, ENT_DATE_COL, ENTITLEMENTS_COL), axis=1)
co_mismatch = df_la.apply(lambda r: check_date_year_mismatch(r, YEAR_COL, CO_DATE_COL, CO_COUNT_COL), axis=1)

any_mismatch = iss_mismatch | ent_mismatch | co_mismatch
df_la_clean = df_la[~any_mismatch].copy()

# Deduplicate so comparison to HCD dashboard is not inflated by duplicate rows
df_la_clean, n_dedup = _deduplicate_apr(df_la_clean)
if n_dedup > 0:
    pct_dedup = 100 * n_dedup / (len(df_la_clean) + n_dedup)
    print(f"APR deduplication: removed {n_dedup:,} duplicate rows ({pct_dedup:.1f}% of pre-dedup LA total)")

# Counts
total_la = len(df_la)
kept_la = len(df_la_clean)
dropped_la = any_mismatch.sum()

# Display statistics
print(f"\n{'='*70}")
print(f"BASICFILTER STATISTICS (Los Angeles only)")
print(f"{'='*70}")
print(f"LA rows loaded:                   {total_la:>10,}")
print(f"LA rows kept:                     {kept_la:>10,} ({100*kept_la/total_la:.1f}%)")
print(f"Dropped (date mismatch):          {dropped_la:>10,} ({100*dropped_la/total_la:.1f}%)")
...
df_la_clean.loc[:, YEAR_COL] = pd.to_numeric(df_la_clean[YEAR_COL], errors='coerce')
df_la_clean.loc[:, PERMITS_COL] = pd.to_numeric(df_la_clean[PERMITS_COL], errors='coerce').fillna(0)
permits_by_year = df_la_clean.groupby(YEAR_COL)[PERMITS_COL].sum()
la_permits = {y: int(permits_by_year.get(y, 0)) for y in permit_years}
